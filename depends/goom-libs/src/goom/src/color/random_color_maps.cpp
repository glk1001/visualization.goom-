    ...
    -> ColorMapSharedPtrWrapper
{
  if (types.empty())
  {
    return ColorMapSharedPtrWrapper{colorMap};
  }

  auto newColorMap = ColorMapSharedPtrWrapper{colorMap};
...
#endif
  {
    newColorMap = GetRandomTintedColorMapPtr(newColorMap.GetColorMapPtr());
  }

...
RELDIR_THIS_FILE = os.path.basename(__file__)

GENERATED_CODE_WARNING = (
    f"// IMPORTANT - THIS CODE IS GENERATED BY:\n"
    f"//\n"
    f"//  '{RELDIR_THIS_FILE}'\n"
    f"//\n"
    f"// DO NOT EDIT!!"
...
def cmap_name_key(text: str):
    keys = [
        conditional_text_to_int(t) for t in re.split(r"(\d+)", get_upper_cpp_name(text))
    ]
...
def get_cmap(name: str) -> Colormap:
    ...
    if cmap is None:
        ...
        cmap = LinearSegmentedColormap.from_list(
            palette.name, palette.mpl_colors, N=palette.number
        )
...
def get_map_types(
    ...
    for m in maps:
        ...
        try:
            ...
            raise Exception(
                'Invalid color map name: "{}" - exception: "{}"'.format(m, e)
            )
...
def write_namespace_end(f):
    f.write(
        f"}} // namespace {GOOM_NAMESPACE}::{COLOR_NAMESPACE}::{COLOR_DATA_NAMESPACE}\n"
    )
...
def write_color_data_maps_cpp(
    color_map_grps: Dict[str, List[str]], used_mps: List[str], dupl: Dict[str, str]
):
    with open(f"{SRCE_DIR}/{FINAL_INCLUDE_RELDIR}/{COLOR_DATA_MAPS_CPP}", "w") as f:
        ...
        for m in used_mps:
            f.write(
                f'#include "{FINAL_PALETTABLE_INCLUDE_RELDIR}/{get_cpp_name(m)}.h"'
        ...
        if len(not_done_maps) > 0:
            print(
                'WARNING: There were uncategorised maps: "{}".'.format(
                    ",".join(not_done_maps)
                )
            )
    ...
    for key in palettable_maps:
        ...
        if key not in color_map_groups[color_group]:
            ...
            if key.lower() == "gray_5" and color_group.lower() == "qualitative":
                ...
                print(
                    f'Fixed palettable key bug: "{key}" from color_group "{color_group}"'

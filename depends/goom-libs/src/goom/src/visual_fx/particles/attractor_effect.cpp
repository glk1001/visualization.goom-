...
static constexpr auto EMIT_RATE_FACTOR_RANGE = NumberRange{0.1F, 0.1F};
...
{
  return EMIT_RATE_FACTOR_RANGE.min * static_cast<float>(m_system.GetNumAllParticles());
...
auto AttractorEffect::AddUpdaters() noexcept -> void
{
  ...
  // NOTE: Order matters - slightly different colors if this is called at start.
  m_system.AddUpdater(m_eulerUpdater);
}

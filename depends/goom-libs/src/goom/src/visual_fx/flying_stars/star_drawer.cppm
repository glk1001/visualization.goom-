...
private:
  ...
  UTILS::MATH::Weights<DrawModes> m_drawModeWeights{
      *m_goomRand,
      {
                  {.key = DrawModes::CLEAN, .weight = DRAW_MODES_CLEAN_WEIGHT},
                  {.key = DrawModes::SUPER_CLEAN, .weight = DRAW_MODES_SUPER_CLEAN_WEIGHT},
                  {.key = DrawModes::MESSY, .weight = DRAW_MODES_MESSY},
                  }
  };

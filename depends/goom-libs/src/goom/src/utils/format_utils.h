...
{
  const auto format = std_fmt::format("({}, {})", fmtStr, fmtStr);
  return fmt::vformat(format, std_fmt::make_format_args(point.x, point.y));
}
...
[[nodiscard]] inline auto RgbF(const char* const fmtStr, const Pixel& pixel)
{
  ...
  const auto format = std_fmt::format("({}, {}, {})", bestFmtStr, bestFmtStr, bestFmtStr);
  return fmt::vformat(format, std_fmt::make_format_args(pixel.RFlt(), pixel.GFlt(), pixel.BFlt()));
}

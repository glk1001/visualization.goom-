...
public:
  WeightedColorMaps();
  explicit WeightedColorMaps(const UTILS::Weights<ColorMapGroup>& weights);

  [[nodiscard]] auto GetWeights() const -> const UTILS::Weights<ColorMapGroup>&;
  void SetWeights(const UTILS::Weights<ColorMapGroup>& weights);
